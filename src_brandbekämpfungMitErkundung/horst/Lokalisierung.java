...
class Lokalisierung {
	...
	byte getDistance(Richtung d) { // dist to next Wall in current Dir
		if (d == Richtung.EAST) {
			for (byte i = 1; i < (map.getlength() - map.getPosX()); i++) {
				...
				if (map.isFree(map.getPosX() + i, map.getPosY())
		...
		if (d == Richtung.SOUTH) {
			for (byte i = 1; i < (map.getlength() - map.getPosY()); i++) {
				...
				if (map.isFree(map.getPosX(), map.getPosY() + i)
						|| coords.contains(coord)) {
					return (byte) (i - 1);
				}
			}
	...
	boolean isFire() {
		...
		for (byte i = 0; i < coords.size(); i++) {
			if ((coords.get(i).getX() - 1) > mX
					|| (coords.get(i).getX() + 1) < mX
					|| (coords.get(i).getY() - 1) > mY
					|| (coords.get(i).getY() + 1) < mY || coords.size() > 5) {
				mX = -1;
				mY = -1;
				return false;
			}
	...
	void nextStep() {
		...
		case EAST:
			x = Math.min(((map.getPosX() + map.getlength()) / 2),
					getDistance(dir));
			if (map.getPosX() == (map.getlength() + 1)/* +1 ? */) {
				dir = changeDir(dir);
				nextStep();
				return;
			}
		...
		case SOUTH:
			x = Math.min(((map.getPosY() + map.getlength()) / 2),
					getDistance(dir));
			if (map.getPosY() == (map.getlength() + 1)) {
				dir = changeDir(dir);
				nextStep();
				return;
			}
		...
		case WEST:
			...
			if (!map.isFree(map.getPosX() - 1, map.getPosY())) {
				dir = changeDir(dir);
				nextStep();
				return;
			}
		...
		case NORTH:
			...
			if (!map.isFree(map.getPosX(), map.getPosY() - 1)) {
				dir = changeDir(dir);
				nextStep();
				return;
			}
	 ...
	 */
	void stepToFire() {
		Koordinaten start = new Koordinaten((byte) map.getPosX(),
				(byte) map.getPosY());
	...
	/**
	 * Recursively finds a path for the robot and returns an ArrayList with the
	 * reverse way.
	 * 
	 * @param from
	 *            the starting point
	 ...
	 * @param dirs
	 *            empty ArrayList to hold the path (in reverse)
	 * @return Whether a path could be found or not.
	 */
	private boolean generatePath(Koordinaten from, Koordinaten to,
			ArrayList<Richtung> dirs) {
		if (from.equals(to)) {
			return true;
		}// Eintraege erstellen
		else if (from.getX() == to.getX()) {// nur noch nach unten
			if (!map.isFree((int) from.getX(), from.getY() + 1)) {// Wand==true
				return false;
			}
			if (generatePath(new Koordinaten(from.getX(),
					(byte) (from.getY() + 1)), to, dirs)) {
				dirs.add(Richtung.SOUTH);
				return true;
			}
		} else if (from.getY() == to.getY()) {// nur noch nach rechts
			if (!map.isFree((int) from.getX() + 1, from.getY())) {// Wand==true
				return false;
			}
			if (generatePath(
					new Koordinaten((byte) (from.getX() + 1), from.getY()), to,
					dirs)) {
