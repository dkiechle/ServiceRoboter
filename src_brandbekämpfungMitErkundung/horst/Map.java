...
public class Map implements IMap {
	...
	/**
	 * Initialisiert die Map. Aktuelle Position wird auf (0,0) gesetzt und
	 * Blickrichtung auf Richtung.NORTH.
	 * 
	 * @param felder
	...
	private byte searchMap(byte x, byte y) {
		for (byte i = 0; i != (byte) map.size(); i++) {
			if (map.get(i).getX() == x && map.get(i).getY() == y)
				return (byte) map.indexOf(map.get(i));
		}
		return -1;
	}
	...
	private byte[] getFeld(int grad, int distanz) {
		...
		case 0:
			return new byte[] {
					(byte) (position[0] + Math
							.floor((distanz
									* Math.sin(((grad * Math.PI) / 180)) / groese) + 0.5)),
					(byte) (position[1] - Math
							.floor((distanz
									* Math.cos(((grad * Math.PI) / 180)) / groese) + 0.5)) };
		case 1:
			grad = grad - 90;
			return new byte[] {
					(byte) (position[0] + Math
	...
	public void setHinderniss(int grad, int distanz) {
		...
		if (loc == -1)
			map.add(new Mapnode(feld[0], feld[1], (byte) 10));
		else {
			Mapnode node = map.get(loc);
			if ((node.getZustand() / 10) == 1 || (node.getZustand() / 10) == 11)
				return;
	...
	public void setFeuer(int grad) { // akktuelle position rausnehmen
		...
		byte t = 0;
		for (byte[] loop = feld.clone(); loop[0] == feld[0]
				&& loop[1] == feld[1]; t++) {
			loop = getFeld(grad, t);
		}
		for (; t != 255; t++) {
			...
			if (loc > -1) {
				...
				if (map.get(loc).getZustand() / 10 == 1
						|| map.get(loc).getZustand() / 10 == 11)
					return;
				for (byte[] loop = feld.clone(); loop[0] == feld[0]
						&& loop[1] == feld[1]; t++) {
					loop = getFeld(grad, t);
	...
	public int getSoll(int grad) {
		...
		if (feld[0] == -1 && feld[1] == -1 || feld[0] == (felder + 1)
				&& feld[1] == -1 || feld[0] == (felder + 1)
				&& feld[1] == (felder + 1) || feld[0] == -1
				&& feld[1] == (felder + 1))
			return 255;
		return distanz;
	...
	public byte getFeuer(int x, int y) {
		...
		if (loc > -1) {
			Mapnode node = map.get(loc);
			if ((node.getZustand() % 10) != 0)
				return (byte) (node.getZustand() % 10);
		}
		return 0;
	}
